...
class TargetLocalizer:
    ...
    def localize(self, frame: RgbdFrame, detection: Detection, min_valid_ratio: float,
        ...
        samples = [frame.depth_m[y][x] for y in range(top, bottom) for x in range(left, right)]
        if not samples:
            return self._fail("empty_depth_roi")
        valid = [float(v) for v in samples if isinstance(v, (int, float)) and math.isfinite(v)
