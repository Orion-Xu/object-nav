...
2. 进入 `Settings`。
3. 进入 `Collaborators`。
4. 点击 `Add people`，输入同事的 GitHub 用户名。
5. 同事在 GitHub 通知或邮箱中接受邀请。
...
同事的显卡型号不必与 RTX 4060 完全相同，但驱动必须足够新，能够运行项目安装的
...
Python 环境、打开 Tkinter GUI，以及支持 OpenCV 图形库。系统询问密码时输入同事
自己的 Ubuntu 登录密码；输入密码时终端不显示字符是正常现象。
...
### 第 4 步：配置同事自己的 Git 姓名和邮箱

下面的姓名和邮箱必须替换成同事自己的信息，不能继续使用徐锦彦的身份：

```bash
git config --local user.name "同事的真实姓名"
git config --local user.email "同事的 GitHub 邮箱"
```
...
`--local` 表示配置只对当前项目生效，不会影响同事电脑上的其他仓库。
...
`.venv/`、`models/`、`data/input/` 和日志已被忽略，不应该进入提交。如果状态中出现
